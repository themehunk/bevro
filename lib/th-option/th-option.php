...
function tab_constant(){
    ...
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'bevro'), esc_html__($theme_data->get( 'Name' )), $theme_data->get( 'Version' ) ),
    'welcome_desc' => esc_html__($theme_data->get( 'Name' ).' is a best multipurpose theme for your project.', 'bevro' ),
...
$pages = get_pages(array(
    ...
    'meta_value' => 'magazine-template.php'
));
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       ...
       'page_template' =>'magazine-template.php', //Sets the template for the page.
    ));
  }
      if($post_id){
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'bevro' ).'</a>
            </div>';
